...
function server() {
  ...
  // 폴링: 스케줄러(외부 cron)가 주기적으로 호출.
  // ✔ 즉시 가벼운 200 응답 후 백그라운드 처리 → cron-job.org "output too large"·콜드스타트 타임아웃 방지.
  // ✔ 동시 실행 차단(락): 스캔 중 들어온 중복 호출은 건너뜀(같은 지연일 중복 전파 방지).
  let scanning = false;
  app.get('/scan', (_, res) => {
    res.json({ ok: true, version: VERSION, busy: scanning });   // 작은 응답 즉시 반환
    if (scanning) return;
    scanning = true;
    scanWorkspace(WORKSPACE_ID)
